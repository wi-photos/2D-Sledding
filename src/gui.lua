...
function GUI:load()
   ...
   self.coins.scale = 0.5
   self.coins.x = love.graphics.getWidth() - 200
   self.coins.y = 50

   self.hearts = {}
   self.hearts.img = love.graphics.newImage("assets/player/penguin.png")
   self.hearts.width = 100
   self.hearts.height = 100
   self.hearts.x = 0
   self.hearts.y = 30
   self.hearts.scale = 0.4
...
function GUI:displayCoinText()
 ...
 --  local y = self.coins.y + self.coins.height / 2 * self.coins.scale - self.font:getHeight() / 2
   local y = self.coins.y + self.coins.height / 2 * self.coins.scale  / 2
   love.graphics.setColor(0,0,0,0.5)
